...
One RV32I image boots to a five-entry menu. UP/DOWN cycles through Pong, Tetris,
the N1 digit screen, the [G2 3D demo](rv32-3d.md) and the G1 2D demo; ENTER starts the selected entry. The
selection wraps over five with explicit compares (a mask worked only for four). ESCAPE returns to the menu. Q ends the session
