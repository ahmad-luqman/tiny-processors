...
| M3 — completed 2026-09-20 ([contract](../rv32-rtl.md), [gates](../rv32-to-gates.md)) | Small multicycle RTL CPU slice: registers, PC, immediate decode, ALU, fetch/execute/writeback, and a documented instruction subset. A short assembly loop agrees with the emulator at retirement, including memory stalls. Lint and synthesis pass without unintended latches. | Map register writes to flip-flops, reads/selects to muxes, and controller states to storage plus combinational next-state logic. Inspect a held request and a single retirement. | 2–4 |
...
## Next implementation session: M4

M3 completed on 2026-09-20; its record is [docs/rv32-rtl.md](../rv32-rtl.md) (contract, controller, retirement port, testbench, verification) and [docs/rv32-to-gates.md](../rv32-to-gates.md) (flip-flops, muxes, controller, waveform, synthesis counts, cycle formula). Its six-step checklist is preserved in Git history and in that document. The core runs eleven instructions, halts on everything else, has no CSRs, and issues word loads only; the testbench is the only memory model.

1. Inspect Git status and preserve every existing command, including the M3 targets. Start a branch and end with one pull request.
2. Extend the contract before the Verilog: the read side of the memory port needs a width or byte-lane request so a byte load of the console status can be told from a word load (M3 recorded the gap); decide it in [docs/rv32.md](../rv32.md) and keep the testbench and the emulator in agreement.
3. Broaden [rtl/rv32/](../../rtl/rv32/) to full RV32I in verified increments: `jalr` and the remaining branches, the other ALU operations and shifts, byte and halfword loads and stores with sign and zero extension, `fence` as a no-op; move each encoding out of the decoder's `unsupported` list as it lands and keep the differential test's unsupported list in step.
4. Add the four CSRs, `csrr*`, `mret`, and trap vectoring so a fault becomes a control transfer instead of a halt; decide the double-fault behavior against the emulator's and write that test first.
5. Run `build/rv32/selfcheck.hex` on the testbench and require `PASS 807d9fad` with the identical 32,610-line trace on both Icarus and Verilator, with and without stalls; add the run to `test-rv32`. Relate its cycle count to the instruction count.
6. Lint, synthesize, record the new cell and flip-flop counts against M3's, capture the waves the roadmap asks for (sign extension, a discarded `x0` write, a stalled store), publish the coverage and limitations table, update this roadmap, and stop before M5's devices.
...
| RTL core microarchitecture and excluded-encoding behavior | Decided in M3: five-state multicycle controller, one port, terminal faults with the emulator's cause codes, `unsupported` halt for valid RV32I not yet implemented | 4 or 5 cycles per instruction plus stalls; 5,777 cells, 1,331 flip-flops; the read side of the port still needs a width, decided in M4 |
