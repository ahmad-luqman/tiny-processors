...
## Next implementation session: M6

M5 completed on 2026-09-21; its record is [docs/rv32-soc.md](../rv32-soc.md) (the bus decoder, the six peripherals, the testbench as host, device time in practice, the diagnostic, three waveforms, synthesis counts) and the contract's [Behavior fixed in M5](../rv32.md#behavior-fixed-in-m5) (the windows, the event word, the checkpoint hash, the device-time rule). The machine now has a timer, a 16-event input queue with a held-key mask, a display controller with a 320×240 framebuffer of 8-bit pixels (fixed RGB332 until a palette exists), and the diagnostic passes on the emulator, Icarus, and Verilator with identical console lines and frame checkpoints; the emulator writes frames as PPM files.

1. Inspect Git status and preserve every existing command, including the M5 targets. Start a branch and end with one pull request.
2. Choose the native window and input library (the deferred choice below) by writing the smallest program that shows a 320×240 RGB332 buffer scaled to the screen and reports key presses; record provenance and the build command. Decide the emulator's host-time timer mode for interactive play (deterministic mode stays the test mode) and the palette question ([docs/rv32.md](../rv32.md), display window `0x2000_3000` reserved).
3. Connect the emulator's framebuffer and input queue to the window: present copies the frame, host keys become the contract's key codes and events, the frame count paces the game; keep the headless mode and every existing option unchanged.
4. Write the guest drawing routines (clear, rectangle, glyphs for digits) and Pong in freestanding C with fixed-point arithmetic; test collision and scoring on the host with the same C compiled natively, as `test_rv32_rt.py` does for the runtime, and replay a scripted input file deterministically on the emulator with frame checkpoints.
5. Replay a bounded scenario on the RTL (Icarus or Verilator) against the same checkpoints; record cycles and wall time; play the game by hand in the window.
6. Lint, synthesize if the RTL changed, update this roadmap, and stop before M7's menu and Tetris.
...
| Native window and input library, host-time timer mode | M6, before the frontend | Minimal library dependencies; integer host scaling; deterministic replay stays the test mode |
