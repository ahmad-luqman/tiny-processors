...
Planning baseline: 2026-09-19. The [interview](full-stack-plan.md) records the twelve user decisions. The destination and ordering are selected; the technical defaults below are proposals to make the next work concrete, not implemented features. No implementation was started during this planning session.
...
flowchart TB
    ...
    subgraph Guest machine contract
        CPU[RV32I CPU] --> BUS[Memory and device interface]
...
Recommended advanced order is A1 → A2 → G1 → N1 → G2 → S1. N1 and G2 are independently reorderable once their prerequisites pass; this does not require more scope questions now. Programmable 3D does not imply a modern shader compiler, full floating-point support, or Vulkan/OpenGL compatibility. Define one meaningful programmable stage before attempting multiple stages or GPU-style scheduling.
...
4. Add startup assembly, linker script, and a tiny freestanding C program testing initialized/uninitialized globals, stack, calls, arithmetic, and a known result. Inspect generated instructions and required helper symbols; supply only the runtime pieces actually needed, with provenance and tests.
5. Execute the firmware on a suitable independent RV32I runner configured for the contract, or a deliberately minimal reference harness if no suitable runner is available. If execution cannot be verified, report the milestone incomplete; an object file or successful link alone does not satisfy M1.
6. Add focused build/validation commands without changing the existing default counter target. Write a C-to-instructions-to-memory walkthrough and a few exercises. Commit the verified milestone in small increments, then walk through it and stop at its boundary.

No RV32I RTL, new GPU feature, or NPU implementation is required to complete M1. Selecting tooling is routine implementation work within the selected architecture; ask again only if evidence requires a material scope change.
