...
| Emulator implementation language and reference runner | Decided in M2: C, one file (a core library plus two mains since M6), host `cc`; QEMU stays the reference and the PC-sequence differential is a make target | Measured about 400 M instructions/s untraced, 1.5 M/s for the Python alternative |
...
| Pretrained model/dataset, weight license, accuracy target | Decided in N1 ([record](../rv32-digit.md)): a 196-32-10 int8 classifier trained once off-line, the MNIST test set vendored with a SHA-256 manifest and a provenance note recording that the source states no license, and a 95% target locked before testing and met at 96.16% | Small classifier whose operations fit the planned engine; lock a test set and accuracy target before acceptance testing |
