...
## Next implementation session: M5

M4 completed on 2026-09-21; its record is [docs/rv32-rtl.md](../rv32-rtl.md) (full instruction set, vectored traps, the double-fault rule, the port with byte strobes both ways, the coverage and limitations table) and [docs/rv32-to-gates.md](../rv32-to-gates.md) (the shared comparator, the barrel shifter, the lane muxes, the trap path, three waveforms, cycle counts, synthesis counts). The core runs all of RV32I plus the four CSRs and `mret`, and the C self-check reaches `PASS 807d9fad` on both simulators with the emulator's 32,610-line trace; the testbench is still the only memory and device model.

1. Inspect Git status and preserve every existing command, including the M4 targets. Start a branch and end with one pull request.
2. Extend the contract before the Verilog: the timer, input, debug output, and framebuffer addresses, widths, and semantics, and the device-time rule (emulator instruction counts are not RTL clock counts), in [docs/rv32.md](../rv32.md); decide the framebuffer resolution and pixel format after measuring the guest's memory and drawing cost.
3. Move the RAM, console, and done register out of the testbench into RTL modules behind a bus decoder that turns an address into a peripheral select, and give the emulator the matched device models; keep every M4 test passing with the same traces.
4. Add the peripherals one at a time with a directed test each (backpressure, error responses, reset during a transfer, timer wrap, input sequencing), on both backends.
5. Write one diagnostic firmware image that exercises every device, run it on the emulator and on the RTL, and compare results and framebuffer checkpoints under the documented time contract; add the run to `test-rv32`.
6. Lint, synthesize, record the counts, capture the waves, update this roadmap, and stop before M6's native window.
