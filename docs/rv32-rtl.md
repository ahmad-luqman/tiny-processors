...
- **Graphics waits**: `+gpu-stall=N` inserts N waits per graphics transfer; `+gpu-seed=S` draws 0..3 waits from an independent seeded generator. They are mutually exclusive and enter the SoC RAM/framebuffer arbitration, independently of CPU `mem_hold`. The runner spells these `--gpu-stall` and `--gpu-seed`.
...
[tools/rv32_rtl.py](../tools/rv32_rtl.py) assembles a program (`--program loop`, the M3 loop, `--program full`, the M4 waves program, or `--program devices`, the M5 one, in [tools/rv32_asm.py](../tools/rv32_asm.py)) or takes a flat image (`--image build/rv32/selfcheck.bin`), runs both backends, diffs the traces, the console text, and the checkpoint lines, and prints how the cycle count follows from the trace; `--mode bench` makes the same comparison for each stall setting and also checks that a fixed stall stalled every transfer. An `--input` script that names one of the run's own files under `--out` is refused before anything is written, because those files are truncated when a backend starts. `--compare results` (M5) skips the trace diff for a program that reads the timer or accesses A2 or G1 command/status/counter registers and compares the console, the outcome, the checkpoints, and the trap records without their step numbers instead; `--input` gives both backends a script, `--expect-last-line` and `--expect-checkpoint` pin the diagnostic's results, and `--backend emulator` runs one side. The tests in [tests/test_rv32_rtl.py](../tests/test_rv32_rtl.py) add directed programs for every instruction class, every trap, every device, and every kind of stop.
...
A2 adds `--simd-stall N` / `--simd-seed S` (testbench `+simd-stall=N` /
`+simd-seed=S`) for independent accelerator data-port delays. CPU `--stall` /
`--seed` remain independent. See the [A2 protocol and counters](rv32-simd4.md).
