...
The ALU grew from 187 cells (one adder) to 1,246: the 33-bit subtractor and its flags, the barrel shifter (five stages of muxes for the left shift and five for the right, with the arithmetic variant sharing the right shifter), and the three logic operations, all under one 8-way result mux. That is the price of RV32I's arithmetic; a real core would share the left and right shifters by reversing the operand, and the exercises below ask you to try it.
