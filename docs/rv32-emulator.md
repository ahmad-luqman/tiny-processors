...
- The **A2 SIMD4** accelerator exposes registers at `0x2000_4000` and program/data memories at `0x2000_5000`/`0x2000_6000`; the **G1 rasterizer** exposes command/status/parameter registers at `0x2000_7000`. Both advance once per executed instruction, including traps. G1 owns the framebuffer and PRESENT while BUSY and protects RAM blit source bytes from CPU writes; display metadata remains readable. See [A2](rv32-simd4.md) and [G1](rv32-gfx.md) for registers and ownership.
