...
- Everything outside RAM and those two devices is an access fault, for fetches as well as data.
...
- stderr ends with one line `rv32emu: halt=<done|double-fault|limit> steps=N retired=N traps=N loaded=N [done=WORD] <outcome>`. The outcome is `pass`, `fail=<code>`, or `error=<reason>`. `steps` counts executed instructions including trapped ones; `retired` excludes them. Nothing here is a cycle count.
...
The call is two instructions because a 32-bit address does not fit one: `auipc` captures the PC, `jalr` adds the offset the linker computed and writes the return address in the same instruction. On entry `sp` was `0x8003_ffc0`: `main` had already taken 64 bytes below `_stack_top` (`0x8004_0000`). `fib` saves `ra` and the three callee-saved registers it will use, then recurses at step 567 with `sp` = `0x8003_ffb0`. Every level costs 16 bytes; the lowest `sp` in the whole trace is `0x8003_fee0`, fourteen frames below `main`'s, which is exactly the depth of `fib(15)` → `fib(1)`. The stack grows downward and only the C compiler's frame arithmetic decides by how much; the machine has no notion of a frame.
...
Limitations that remain: no timer, input, or framebuffer (M5); no interrupts, `mstatus`, or privilege modes; no cycle timing of any kind; trace mode is about fifty times slower than plain execution. The next milestone, M3, is the multicycle RTL CPU slice whose retirement trace must match this emulator's line for line on a short assembly loop.
