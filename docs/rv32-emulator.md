...
- `--checkpoints`, `--frames`, and `--input` (M5) are described under the machine model; a malformed script line, an unopenable script, or an unwritable frame file is an emulator error (exit 2), and the checkpoints path may not name the image, the trace, or the state file.
