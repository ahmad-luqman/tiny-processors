...
make run-rv32-gfx-menu-emu run-rv32-gfx-menu-rtl-verilator
...
invalid commands, busy submission, timeout reset, and relaunch. It prints
...
and packed spans are future optimization opportunities.

Standalone synthesis: **52,444 generic cells, 1,138 flip-flops**, no latches.
The complete SoC with the existing 64-word synthesis RAM/framebuffer substitutions
has **165,285 generic cells, 22,980 flip-flops**. Those substitutions measure logic
...
The generated JSON separately records CPU instruction/command/FB traffic, engine
...
| --- | ---: | ---: | ---: | ---: |
| Full 320×240 fill | 339,677 | 230,650 | 230,401 | 0 / 76,800 |
| 32×32 RAM blit | 294,490 | 4,531 | 4,300 | 1,024 / 1,024 |
| 51-pixel line | 4,796 | 394 | 154 | 0 / 51 |
| Triangle, 975 covered pixels | 92,525 | 5,224 | 4,976 | 0 / 975 |

RAM blit includes 203 arbitration wait cycles even with zero injected waits.
Adding two waits per CPU and graphics transfer gives accelerator-path costs of
384,429 / 8,594 / 654 / 7,320 cycles. The CPU baseline uses word-packed fills;
...
230,451, since the emulator advances the device once per instruction. They are
...
- `reset.vcd`: the third write is held from 435 ns. RESET removes valid at
  450 ns before its accepting edge; status/counters clear at 455 ns. Two earlier
