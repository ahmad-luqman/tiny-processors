...
`mem_ready` is `req` AND the OR of each select ANDed with its slave's `ready` (plus `none_sel`); `mem_error` is the OR of each select ANDed with its slave's `error` (plus `none_sel`); `mem_rdata` is a one-hot AND-OR mux of the slaves' read data. Adding a slave is one select, one `_valid`, and one term in each of the three ORs. Synthesis makes 525 cells of it, all logic.

### The devices

- **RAM** ([rv32_ram.v](../rtl/rv32/rv32_ram.v)): `WORDS` words, an asynchronous read of the whole aligned word (a continuous assign, so the array never enters a sensitivity list) and a strobe-masked write at the accepting edge, exactly the testbench's old model. There is no initial block: the testbench zero-fills and loads the image through the hierarchy, and synthesis never sees a file. The same module is the framebuffer with `WORDS = 19200`, behind its own window: a pixel store is data movement and nothing else.
...
- **Display** ([rv32_display.v](../rtl/rv32/rv32_display.v)): PRESENT raises `present` for the accepting cycle and counts a frame; FRAMES, WIDTH, and HEIGHT are read-only words. The pixels are not here: the host snapshots the framebuffer memory when it sees `present`, which is all "presenting" means in M5. In M6 the native window will do the same.
...
| --- | ---: | ---: | --- |
| `rv32` core tree | 8,272 | 1,457 | 8,175 alone (`make synth-rv32`, unchanged); ABC sees a different netlist inside the SoC |
...
Limitations that remain: no palette (the display window at `0x2000_3000` is reserved for one in M6) and no host-time timer mode (M6 chooses one for interactive play; deterministic mode stays the test mode); no interrupts, so waiting is polling; the input script is the only keyboard until the native window exists; the RTL's memories are flip-flops in synthesis, not macros. M6 builds the native Mac window on the emulator's framebuffer and input queue, software drawing, and Pong, and replays a bounded scenario on the RTL against these checkpoints.
