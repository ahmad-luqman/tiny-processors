...
The program interface reads one of 256 **32-bit words** combinationally. The separate data interface addresses 256 **16-bit words** through one shared port. Addresses and PC wrap modulo 256; ADD/ADDI/MUL wrap modulo 65536. Signed and unsigned addition, and the low 16 bits of a product, are identical bit patterns. MAC and MACU differ: they extend both 16-bit operands to 32 bits with sign or zero and add the full 32-bit product into the accumulator modulo 2^32. Nothing saturates and nothing rounds; RDA truncates. A saturating accumulate and a rounded read-back were written down as N1 choices; N1 answered them without changing this hardware. Saturation and rounding are CPU operations after the exact 32-bit accumulator is read back through `RDA 0` and `RDA 16`, so the engine stays exact and wrapping and the N1 exporter proves no launch can wrap. See [the digit record](rv32-digit.md). A saturating read-back and a rounded shift remain exercises.
...
`test-simd4` and `test-simd4-verilator` each pass 395 cases and 423 completed launches, with 24 additional reset-aborted prefixes and one negative case the testbench must refuse. Coverage includes every illegal opcode faulting after a MAC has loaded the accumulators (four of them, plus `SETLOOP 0` and `LOOP` with a zero counter, also on one and two lanes), invalid loop counts, arithmetic/address/PC wraparound, register aliases, colliding stores, shared loops, ignored starts while busy, sticky done/fault, relaunch without reset, variable backpressure, partial-load reset, partial-store reset, reset with live accumulators after a MAC, inside a matrix row, and while a MAC or an RDA is executing, deterministic mixed instruction sequences with and without the accumulator opcodes, the hand-computed extreme products in one image together with the fields CLRA, RDA, MAC and MACU ignore, instruction bits [17:16] set, and lane-distinct MUL/MACU operands, a read-back sweep of all 32 shifts plus out-of-range immediates on negative and positive accumulators, and 2×2, 4×4 and 8×8 matrix kernels with ordinary and corner operands: the corner matrices rotate all four corners through the other rows of A and columns of B, hold row 0 and column 0 at `7fff`, and so make C[0][0] pass 2^31 (4×4) or wrap past 2^32 (8×8), read back at shift 16 for every size and shift 31 for 4×4. The testbench compares every retirement (registers and accumulators) and accepted transfer, all final memory words, stalled request stability, and performance counters. Each fixture carries its snapshot and record widths, which the testbench checks against its own with four-state comparisons (a two-lane fixture through the four-lane binary is the suite's negative control), and the runner rejects fixture rows wider than their array and any simulator warning or error, since neither simulator fails on them (Icarus only warns, Verilator says nothing). Twenty-one Python tests pin encodings, the snapshot layout, extreme products, kernel words, predicted instruction/transfer counts, input validation, and the runner's own guards.
...
- Both simulators pass 395 cases and 423 completed launches; the retirement comparison now includes every accumulator. Lint is clean for one, two and four lanes; four-lane synthesis has no latches.
...
Still deferred: a saturating accumulate and a rounded read-back *in hardware* (N1 settled the numeric contract by doing both on the CPU after an exact two-half read-back, leaving this engine unchanged), a broadcast load that fills every lane from one transfer, a second loop counter, divergent branches, tail masks, and physical FPGA memory integration. CPU-commanded launch through the RV32 bus, private shared buffers and a driver are completed in [A2](rv32-simd4.md); the standalone engine above is unchanged.
