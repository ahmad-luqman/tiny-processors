...
Still deferred: a saturating accumulate and a rounded read-back (N1 numeric contract), a broadcast load that fills every lane from one transfer, a second loop counter, divergent branches, tail masks, and physical FPGA memory integration. CPU-commanded launch through the RV32 bus, private shared buffers and a driver are completed in [A2](rv32-simd4.md); the standalone engine above is unchanged.
