...
module fp32 (
    ...
    function increment;
        ...
        begin
            case (rounding)
                0: increment = guard_bit && (sticky || odd);
                1: increment = 0;
                2: increment = negative && (guard_bit || sticky);
                3: increment = !negative && (guard_bit || sticky);
                4: increment = guard_bit;
                default: increment = 0;
            endcase
        end
    endfunction
    ...
    wire product_sign = x[31] ^ y[31] ^ (operation == 5 || operation == 6);
    wire third_sign = z[31] ^ (operation == 4 || operation == 6);
    wire second_sign = y[31] ^ (operation == 1);
    wire fused = operation >= 3 && operation <= 6;
    ...
    wire overflow_inf = mode == 0 || mode == 4 ||
                              (mode == 2 && sign_result) || (mode == 3 && !sign_result);
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            operation <= 0; mode <= 0; x <= 0; y <= 0; z <= 0;
            ...
            case (state)
                ...
                DECODE: begin
                    if (operation > 17 || mode > 4) finish(0,0,1);
                    else if (operation == 0 || operation == 1) begin
                        if (is_nan(x) || is_nan(y))
                            finish(32'h7fc00000,{is_snan(x)||is_snan(y),4'b0},0);
                        else if (is_inf(x) && is_inf(y) && x[31] != second_sign)
                            finish(32'h7fc00000,5'h10,0);
                    ...
                    end else if (operation >= 2 && operation <= 6) begin
                        ...
                        else if (is_zero(y)) finish({x[31]^y[31],8'hff,23'b0},5'h08,0);
                    ...
                    end else if (operation == 9 || operation == 10) begin
                        sign_result <= operation == 9 && x[31];
                        magnitude <= {((operation == 9 && x[31]) ? (~x + 32'd1) : x),544'b0};
                        exponent <= 12'sd31; state <= NORMALIZE;
                    end else if (operation == 11 || operation == 12) begin
                        sign_result <= x[31];
                        if (is_nan(x)) finish(operation == 12 ? 32'hffffffff : 32'h7fffffff,5'h10,0);
                        else if (is_inf(x) || exp_x > 31)
                            finish(operation == 12 ? (x[31] ? 32'b0 : 32'hffffffff) :
                                   (x[31] ? 32'h80000000 : 32'h7fffffff),5'h10,0);
                        else begin
                            magnitude <= {sig_x,552'b0}; exponent <= exp_x;
                            state <= INT_SHIFT;
                        end
                    end else if (operation >= 13 && operation <= 15) begin
                        if (is_nan(x) || is_nan(y))
                            finish(0,{operation != 13 || is_snan(x) || is_snan(y),4'b0},0);
                        else finish({31'b0,(operation == 13 ? eq_xy : operation == 14 ? lt_xy : (eq_xy||lt_xy))},0,0);
                        ...
                        else if (is_zero(x) && is_zero(y)) finish(operation == 16 ? (x|y) : (x&y),0,0);
                        else finish((lt_xy ^ (operation == 17)) ? x : y,0,0);
                    end
                end
                ALIGN: begin
                ...
                COMBINE: begin
                    if (sign_result == sign_addend) magnitude <= magnitude + addend;
                        ...
                        if (magnitude == addend) sign_result <= mode == 2;
                ...
                ROUND: begin
                    if (final_exponent > 127)
                        finish({sign_result,overflow_inf ? 31'h7f800000 : 31'h7f7fffff},5'h05,0);
                    else finish({sign_result,packed_exponent,final_significand[22:0]},
                                {3'b0,discarded && tiny_after_rounding,discarded},0);
                end
                DIVIDE: begin
                ...
                INT_ROUND: begin
                    if (integer_invalid) finish(integer_limit,5'h10,0);
