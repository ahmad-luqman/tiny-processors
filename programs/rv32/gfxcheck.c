...
static int check(struct gpu_command *c,const uint8_t *source)
{
    ...
    if(!gpu_run(c,1000000))return 0;
    volatile uint8_t *fb=(volatile uint8_t *)RV32_FB_BASE;
    for(uint32_t i=0;i<sizeof expected;i++)if(fb[i]!=expected[i]){rv32_puts("G1 pixel mismatch at ");rv32_put_hex32(i);rv32_putc('\n');return 0;}
    return 1;
}
int main(void)
{
    ...
    /* Fault diagnostics, BUSY submit refusal, zero/nonzero poll-budget expiry,
     * timeout snapshot survival, then a valid zero-sized relaunch. */
    gpu_command_init(&c,9,0);
    if(!gpu_submit(&c) || gpu_wait(100)!=GPU_FAULT || mmio_read32(GPU_BASE+GPU_STATUS)!=GPU_FAULT || mmio_read32(GPU_BASE+GPU_WRITES)!=0 || gpu_last_result()->reason!=GPU_INVALID){rv32_puts("G1 fault diagnostic failed\n");return 7;}
    gpu_reset();gpu_command_init(&c,GPU_FILL,0);c.p[GP_W]=320;c.p[GP_H]=240;
    if(!gpu_submit(&c) || gpu_submit(&c) || gpu_wait(0)!=GPU_TIMEOUT || mmio_read32(GPU_BASE+GPU_STATUS)!=0){rv32_puts("G1 zero-budget reset failed\n");return 8;}
    if(gpu_last_result()->status!=GPU_BUSY || !gpu_submit(&c) || gpu_wait(1)!=GPU_TIMEOUT ||
       gpu_last_result()->status!=GPU_BUSY || gpu_last_result()->cycles==0 || mmio_read32(GPU_BASE+GPU_STATUS)!=GPU_IDLE){rv32_puts("G1 timeout snapshot failed\n");return 10;}
    c.p[GP_W]=0;
    if(!gpu_submit(&c) || gpu_wait(100)!=GPU_DONE || mmio_read32(GPU_BASE+GPU_WRITES)!=0){rv32_puts("G1 empty relaunch failed\n");return 9;}
    rv32_puts("PASS G1\n");return 0;
}
