...
from tools.simd4_model import ADD, ADDI, CLRA, HLT, LANE, LDI, LOAD, LOOP, MAC, RDA, SETLOOP, STORE, image, word
...
def reference(a, b, n, shift=0):
    ...
    for i in range(n):
        for c in range(n):
            acc = sum(signed(a[i * n + k]) * signed(b[k * n + c]) for k in range(n)) % 2**32
            acc = acc - 2**32 if acc & 0x80000000 else acc
            out.append((acc >> shift) % 65536)
    return out
