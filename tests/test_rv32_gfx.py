...
RAM_PATTERN=bytes((i*17+3)&255 for i in range(262144))
U32=C.c_uint32
...
class Graphics(unittest.TestCase):
    ...
    def access(self,off,v=None,width=4,ok=True):
        ...
        return value.value
    def submit(self,p):
        for i,v in enumerate(p):self.access(GPU_PARAMS+4*i,v&0xffffffff)
        self.access(GPU_COMMAND,GPU_START);self.lib.native_gpu_tick(0) # command tick
    def run_command(self,p,mode=0,abort=-1):
        self.submit(p);ticks=0
        while self.access(GPU_STATUS)==GPU_BUSY:
            if ticks==abort:
                if mode&2:self.lib.native_gpu_reset()
                else:self.access(GPU_COMMAND,GPU_RESET)
            self.lib.native_gpu_tick(mode&1 and (ticks%7==2 or ticks%7==4));ticks+=1
            self.assertLess(ticks,4000000)
        return [fnv(C.string_at(self.lib.native_gpu_fb(),76800))]+[self.access(o) for o in (GPU_STATUS,GPU_ERROR,GPU_CYCLES,GPU_STALLS,GPU_READS,GPU_WRITES)]
    def test_pixels_and_rtl(self):
        ...
        for x,y in [(8,3),(3,8),(-8,3),(-3,8),(8,-3),(3,-8),(-8,-3),(-3,-8),(0,0),(0,8),(8,0),(2,1),(-2,1),(2,-1),(-2,-1),(1,2),(-1,2),(1,-2),(-1,-2)]:
        ...
        cmds += [command(4,x0=300,y0=220,x1=350,y1=225,x2=305,y2=260),
                 command(4,x0=-1024,y0=-1024,x1=1023,y1=-1024,x2=0,y2=1023),
                 command(4,x0=300,y0=220,x1=305,y1=260,x2=350,y2=225),
                 command(4,x0=1,y0=1,x1=5,y1=1,x2=1,y2=5,w=0x7fffffff,h=0x7fffffff),
                 command(4,x0=-1,y0=-1,x1=5,y1=1,x2=1,y2=5,w=0x80000000,h=0x80000000),
                 command(x0=500,y0=500,w=10,h=10,x1=0x7fffffff),
                 command(3,x0=500,y0=500,x1=510,y1=520),
                 command(4,x0=500,y0=500,x1=510,y1=500,x2=500,y2=520)]
        # Distinct directed colours expose dropped pixels in reversed/overlapping jobs.
        for n,p in enumerate(cmds):p[1]=(n*37+11)&255
        ...
        for _ in range(24):
            cmds.append(command(2,x0=rng.choice([-8,0,310]),y0=rng.choice([-6,0,230]),w=20,h=20,
                src=0x80000000+rng.choice([0,1,2,3,0xff,0x1000,0x1fffd,0x3fd00]),stride=19,sw=17,sh=15,sx=rng.randrange(-5,20),sy=rng.randrange(-5,20)))
        records=[];rows=[]
        for n,p in enumerate(cmds):
            ...
            self.assertEqual(rec[1:3],[GPU_DONE,0])
            self.assertEqual(rec[5:], [writes if p[0]==2 else 0,writes], (n,p))
            if n%2 and writes>1:self.assertGreater(rec[4],0,(n,p))
            records.append(rec);rows.append((n%2,-1,p))
        def invalid(p):
            before=C.string_at(self.lib.native_gpu_fb(),76800)
            rec=self.run_command(p)
            self.assertEqual(rec[1:3],[GPU_FAULT,1])
            self.assertEqual(rec[4:],[0,0,0])
        ...
        p=command(w=3,h=2)
        before=bytearray(C.string_at(self.lib.native_gpu_fb(),76800));self.assertEqual(oracle(before,p),6)
    ...
    def test_mmio_ownership(self):
        ...
        self.access(GPU_COMMAND,GPU_RESET);self.assertEqual(self.access(4),0);self.assertEqual(self.access(64),0)
        for p in (command(2,w=1,h=1,src=0x80001000,stride=16,sw=16,sh=0),
                  command(2,w=1,h=1,src=0xfffffff0,stride=16,sw=16,sh=2)):
            self.submit(p)
            self.assertEqual(self.lib.native_gpu_lock(p[GP_SRC],1),0)
