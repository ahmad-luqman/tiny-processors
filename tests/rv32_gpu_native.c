...
int main(int argc,char **argv)
{
    ...
    if(argc==2){
        ...
        while((rc=fscanf(input,"%d %d",&mode,&abort_tick))==2){
            if(mode<0 || mode>3 || abort_tick < -1 || abort_tick>4000000)return 18;
            struct gpu_command c;
            for(unsigned i=0;i<16;i++){
                char token[10];
                if(fscanf(input,"%9s",token)!=1 || strlen(token)!=8)return 11;
                for(unsigned k=0;k<8;k++)if(!isxdigit((unsigned char)token[k]))return 11;
                c.p[i]=(uint32_t)strtoul(token,0,16);
            ...
            while(g.status==GPU_BUSY){
                ...
                gpu_tick(&g,ram,sizeof ram,fb,(mode&1) && (ticks%7==2 || ticks%7==4));
            ...
            if(g.status==GPU_FAULT){
                if(g.writes || g.reads || g.stalls || memcmp(before,fb,sizeof fb))return 19;
            }
            jobs++;
        }
