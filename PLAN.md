...
Updated 2026-09-22 after the [planning interview](docs/planning/full-stack-plan.md) and the completed M1 to M7, F1, F2 and A1 sessions. The machine contract, the firmware toolchain, the emulator with its native window, a full RV32I multicycle RTL CPU, the machine around it (bus decoder, timer, input queue, display and framebuffer, matched on both backends), Pong, the boot menu/runtime/Tetris capstone, complete RV32F CPU/emulator integration, and a SIMD4 engine that multiplies and accumulates exist; no accelerator is attached to the new computer's bus yet.
...
These rows record each milestone’s acceptance baseline; the F1 session also reran the preserved machine and lab regressions. SAP8 stays frozen as the small teaching CPU. SIMD4 is a parallel compute prototype, not an integrated GPU or NPU. Its multiply/matrix extension (A1) is complete; A2 attaches it to the RV32 bus.
...
A1 fixed SIMD4's arithmetic contract (16×16→32 signed or unsigned products,
a per-lane 32-bit wrapping accumulator, truncating read-back), verified it
against hand-computed extremes on both simulators, and ran a square matrix
kernel whose transfers do not fall with lane count because every lane loads
the shared operand itself. See the [A1 record](docs/simd4.md#a1-acceptance-record-2026-09-22).

Next, A2: map the engine's launch, status, program and data memories into the
RV32 machine's address space behind the bus decoder, write the guest driver
that loads a kernel and polls for completion, define CPU/device buffer
ownership and exactly-once memory effects, and test stalled memory, reset
during a transfer, and error reporting on the emulator and the RTL.
