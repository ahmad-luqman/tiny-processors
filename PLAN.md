...
Updated 2026-09-22 after the [planning interview](docs/planning/full-stack-plan.md) and the completed M1 to M7, F1 and F2 sessions. The machine contract, the firmware toolchain, the emulator with its native window, a full RV32I multicycle RTL CPU, the machine around it (bus decoder, timer, input queue, display and framebuffer, matched on both backends), Pong, the boot menu/runtime/Tetris capstone, and complete RV32F CPU/emulator integration exist; no accelerator for the new computer does yet.
...
| RV32F CPU and emulator (F2) | Complete F/Zicsr state and retirement; nine architectural tests on both simulators, 1,080 seeded CPU vectors and 45 literal anchors in static/dynamic modes; ILP32 C float programs, RV32I software benchmark; 42,034-cell CPU, latch-free | [F2 contract and walkthrough](docs/rv32-f.md); `make test-rv32-f`, `make bench-rv32-f`, `make waves-rv32-f` |
...
F2's complete RV32F CPU/emulator integration is verified: floating registers,
Zicsr aliases, every F instruction, exact rounding/flags, reset cancellation,
ILP32 C float programs and an RV32I software benchmark. See the
[F2 record](docs/rv32-f.md). The same arithmetic workload takes 6,037 RTL cycles
with F versus 80,924 with guest software floating point, without memory stalls.

Next, define multiply/accumulate widths and a small matrix kernel, building on
SIMD4. Compare edge and ordinary cases against a software model, report
transfers/cycles/stalls, and retain the original vector-add artifact. CPU
commands and shared-buffer/device integration follow in A2.
