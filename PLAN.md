...
Updated 2026-09-21 after the [planning interview](docs/planning/full-stack-plan.md) and the completed M1 to M5 sessions. The machine contract, the firmware toolchain, the headless emulator, a full RV32I multicycle RTL CPU, and the machine around it (bus decoder, timer, input queue, display and framebuffer, matched on both backends) exist; no native window, OS, or accelerator for the new computer does yet.
...
Give the [RV32 machine](docs/rv32.md) a native Mac window: present the emulator's framebuffer, turn host keys into the contract's key events, choose a host-time timer mode for play while deterministic replay stays the test mode; then software drawing routines and Pong in freestanding C, tested on the host, replayed deterministically on the emulator against frame checkpoints, and replayed as a bounded scenario on the RTL. See the [next-session checklist](docs/planning/roadmap.md#next-implementation-session-m6).
